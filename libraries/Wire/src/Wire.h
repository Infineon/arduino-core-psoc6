...
private:
    ...
    uint8_t rxBuffer[BUFFER_LENGTH] = {0};
    uint8_t txBuffer[BUFFER_LENGTH] = {0};
    size_t rxBufferIndex = 0;
    size_t rxBufferLength = 0;
    size_t txBufferIndex = 0;
    size_t txBufferLength = 0;
    uint8_t txAddress = 0;
    cyhal_i2c_cfg_t i2c_config = {CYHAL_I2C_MODE_MASTER, 0, I2C_DEFAULT_FREQ};
    cyhal_i2c_t i2c_obj;
    cy_rslt_t w_status = CY_RSLT_SUCCESS;
