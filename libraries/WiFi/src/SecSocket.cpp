...
void Socket::begin(socket_protocol_t protocol) {
    ...
    _last_error = cy_socket_create(CY_SOCKET_DOMAIN_AF_INET, socket_type, socket_proto, &socket);
    socket_assert(_last_error);
...
bool Socket::joinMulticastGroup(IPAddress multicastIP, IPAddress localIP) {
    ...
    mreq.multi_addr.ip.v4 =
        (multicastIP[3] << 24) | (multicastIP[2] << 16) | (multicastIP[1] << 8) | multicastIP[0];
    mreq.if_addr.version = CY_SOCKET_IP_VER_V4;
    mreq.if_addr.ip.v4 = (localIP[3] << 24) | (localIP[2] << 16) | (localIP[1] << 8) | localIP[0];

    cy_rslt_t result = cy_socket_setsockopt(socket, CY_SOCKET_SOL_IP,
                                            CY_SOCKET_SO_JOIN_MULTICAST_GROUP, &mreq, sizeof(mreq));
...
uint32_t Socket::send(const void* data, uint32_t len, IPAddress ip, uint16_t port) {
    ...
    _last_error = cy_socket_sendto(socket, data, len, CY_SOCKET_FLAGS_NONE, &address,
        ...
        switch (_protocol) {
            ...
            case SOCKET_PROTOCOL_UDP:
                _last_error = cy_socket_recvfrom(socket, temp_rx_buff, bytes_rcvd_request,
                                                 CY_SOCKET_FLAGS_RECVFROM_NONE, peer_addr, nullptr,
                                                 &bytes_received);
                break;
            default:
                _last_error = SOCKET_STATUS_UNINITED;
